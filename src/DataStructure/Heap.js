export function defaultCmp(x, y) {
    if (x < y) {
        return -1;
    }
    if (x > y) {
 ...
 * @param {Function} cmp 
 */
export function heapDown(arr, pos, cmp) {
 ...
 * @param {Function} cmp 
 */
export function heapUp(arr, pos, cmp) {
    if (!cmp) cmp = defaultCmp;
    var item = arr[pos];
    var parPos;
    while (pos > 0) {
 ...
 * @param {Function} cmp
 */
export function heapify(arr, cmp) {
    if (!cmp) cmp = defaultCmp;
    var endPos = arr.length;
 ...
 * @param {Function} cmp 
 */
export function heapPop(arr, cmp) {
    if (!cmp) cmp = defaultCmp;
    var item = arr[0];
 ...
 * @param {Function} cmp 
 */
export function heapPush(arr, item, cmp) {
    if (!cmp) cmp = defaultCmp;
    arr.push(item);
